...
| `shutup.json` | `Box Blur2`와 `Bevel Alpha` effect path는 둘 다 실제로 돈다. 이번 배치에서 `Box Blur2` radius가 `thisComp.layer('SIZE').transform.scale[0]`를 따르는 좁은 expression subset까지 붙어서 frame 0 adjudication이 약 `0.652 -> 0.688`로 올라갔다. 다만 steady-state는 여전히 ThorVG보다 느리고 mixed layer-effect semantics도 아직 ThorVG와 꽤 다르다. |
