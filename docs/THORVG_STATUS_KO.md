...
| `ADBE Box Blur2` | narrow whole-layer bitmap postprocess로 추가했다. 현재는 static radius/iterations/dimensions와 `Repeat Edge Pixels=0` subset을 지원하고, 추가로 `value * thisComp.layer(...).transform.scale[0] / 100` 형태의 좁은 radius expression subset도 처리한다. synthetic fixture `layer_effect_box_blur_solid.json`은 nonzero pixels를 `400 -> 784`로 넓히고, `shutup.json`에서는 blur radius가 `SIZE.scale[0]`를 따라간다 |
| `ADBE Bevel Alpha` | narrow whole-layer bitmap postprocess로 추가했다. 기본 static `Edge Thickness / Light Angle / Light Color / Light Intensity` subset은 `layer_effect_bevel_alpha_solid.json`에서 실제 lighting을 만들고, 이번 배치에서 `Edge Thickness`도 `value * thisComp.layer(...).transform.scale[0] / 100` 형태의 좁은 scale-driven expression subset을 읽는다. `layer_effect_bevel_alpha_scale_expr.json`은 equivalent static-expanded fixture와 byte-identical이다 |
...
| 부분 지원 | ADBE Box Blur2 | 좁은 지원 | 지원 관찰 | static radius/iterations/dimensions와 `Repeat Edge Pixels=0` subset은 synthetic fixture에서 실제 coverage를 넓히고, `value * layer.scale[0] / 100` radius expression subset도 `shutup.json`에서 동작한다 | broader parameter coverage와 mixed stack 검증 |
| 부분 지원 | ADBE Bevel Alpha | 좁은 지원 | 지원 관찰 | `Edge Thickness`, `Light Angle`, `Light Color`, `Light Intensity` subset은 synthetic fixture에서 lighting을 만들고, `value * layer.scale[0] / 100` edge-thickness expression subset도 static-expanded reference와 정확히 일치한다 | broader lighting semantics와 mixed stack 검증 |
...
- `shutup.json`은 이제 `Box Blur2`만 있는 asset이 아니라 `Bevel Alpha` 경로까지 실제로 돈고, blur radius가 `SIZE.scale[0]`를 따르는 좁은 expression subset도 반영된다. 그래도 frame 0 exact match가 겨우 `0.688` 수준이라, narrow effect 추가를 곧바로 “실자산 해결”로 해석하면 안 된다.
