...
| `world_locations.json` matte 경로 | 기존 `ShapeLayer` alpha offscreen clip tightening 위에, single solid fill의 uniform alpha도 direct-alpha matte로 처리하도록 넓혔다. `first-frame` 정합성은 그대로 유지된다 |
| `world_locations.json` 최신 판정 | first-frame adjudication은 여전히 `exact_match_ratio = 0.999722` 수준이다. 최신 profile에서는 `render_matte_ms = 15.05 ms`까지 내려왔고, hardened median steady-state는 `0.514 ms -> 0.239 ms(ThorVG)`다. 같은 머신에서 `HEAD baseline`과 교차 A/B median을 다시 재면 `0.521 ms -> 0.482 ms`로 내려간다 |
...
| rejected 실험 | recursive precomp `coverageBounds()`, `11555/confetti/threads`용 translation-only RLE 재사용, `stroke_dash`용 narrow `ADBE 4ColorGradient`, broad `contentStatic` skip은 모두 benchmark/adjudication에서 살아남지 못해 버렸다 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| world_locations.json | matte/offscreen | 0.267 | 0.563 | 0.514 | 0.239 | 2.15x | 9840 | 5040 | ThorVG 우세 |
| 11555.json | transform cache | 0.201 | 0.488 | 1.541 | 1.409 | 1.09x | 4528 | 3680 | ThorVG 우세 |
| confetti.json | transform cache | 1.613 | 0.669 | 0.209 | 0.109 | 1.92x | 4016 | 4368 | ThorVG 우세 |
| threads.json | transform cache | 0.133 | 0.513 | 2.013 | 2.017 | 1.00x | 4800 | 3648 | 사실상 동률 |
| text_anim.json | outlined text scene | 1.165 | 0.543 | 0.129 | 0.087 | 1.48x | 3936 | 4160 | ThorVG 우세 |
| stroke_dash.json | real text + effect | 0.136 | 0.454 | 0.159 | 0.126 | 1.26x | 3936 | 3392 | ThorVG 우세 |
| textrange.json | real text animator | 0.135 | 0.433 | 0.008 | 0.029 | 0.27x | 2800 | 3344 | rlottie 우세 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| matte/offscreen | world_locations.json | 0.267 | 0.563 | 0.514 | 0.239 | 2.15x | 9840 | 5040 | ThorVG 우세 |
| transform cache | 11555.json, confetti.json, threads.json | 0.649 | 0.557 | 1.254 | 1.178 | 1.06x | 4448 | 3899 | ThorVG 우세 |
| outlined text scene | text_anim.json, textblock.json | 2.831 | 0.792 | 0.489 | 0.203 | 2.41x | 6264 | 5872 | ThorVG 우세 |
| real text / text animator | stroke_dash.json, textrange.json | 0.126 | 0.447 | 0.082 | 0.082 | 1.01x | 3344 | 3368 | 사실상 동률, correctness는 rlottie 열세 |
...
| 우선순위 | 대상 | 이유 | 바로 할 작업 |
| --- | --- | --- | --- |
| 1 | `expressions/world_locations.json` | first-frame은 그대로 맞고, translucent single-fill direct matte까지 넣은 뒤에도 steady-state 격차가 가장 크다 | inherited bounds 전파 마무리, matte 재사용, multi-drawable translucent matte로 살아남는 경로만 재탐색 |
| 2 | `11555.json`, `confetti.json`, `threads.json` | `matrix dirty -> shape dirty -> reraster` 체인이 본질이다. 이번 배치에서 `threads`는 거의 따라잡았지만 `11555`와 `confetti`는 여전히 남는다 | content/transform dirty 분리, local-space snapshot cache를 더 좁은 조건으로 재설계 |
...
- 이번 `single translucent fill direct matte` 변경은 같은 머신에서 `HEAD baseline`과 교차 A/B median으로 다시 확인했다. `world_locations`는 `0.521 -> 0.482 ms`, `11555`는 `1.530 -> 1.429 ms`, `threads`는 `2.126 -> 1.999 ms`로 내려갔고, `confetti`만 `0.183 -> 0.186 ms`로 소폭 나빠졌다.
